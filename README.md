...
`acquisition-batch-result.v1` emits exactly one terminal disposition with a stable non-empty reason per externally requested site/seed. Agentic link/search discoveries and retries remain snapshot/failure evidence rather than becoming extra requested-site dispositions. Its requested/succeeded/failed/unresolved counts are therefore independent from valid-snapshot and failed-evidence counts, and `authoritative_status` prevents a partial/cancelled/failed parent run from being projected as full success. Successful dispositions may carry immutable artifact IDs; failed or unresolved dispositions never do. HTTP failures, high-confidence block/CAPTCHA phrases, explicit same-origin block endpoints, empty extracted pages, and acquisition exceptions remain failed evidence without successful content. Generic brand/UI text and ordinary topic paths such as `/research/challenge/...` do not by themselves turn substantive content into failed evidence.

`AgenticRunResult.to_acquisition_batch_result()` is the read-only projection of the Agentic ledger. The supported governed `run-scope --json` path also includes the same strict result in its existing persisted `job_delivery.v1` record. A rejected initial governed seed or a later page/file acquisition failure persists the job as failed, emits the canonical counts and disposition, and exits nonzero; only a fully successful requested site keeps the completed job and zero exit. Arbitrary runtime and rollback-required authority failures remain their existing generic or governed command errors rather than being mislabeled as site acquisition results. This result is directly consumable by a later manifest producer but is not itself the Issue #54 exporter, a second source of truth, a new batch command, or new network authority. Legacy `check` and direct browser target reads remain disabled.
...
- `acquisition-batch-result.v1` (read-only terminal projection of Agentic task/observation/artifact lineage)
...
- [acquisition-batch-result-v1.sample.json](docs/testing/fixtures/acquisition-batch-result-v1.sample.json)
