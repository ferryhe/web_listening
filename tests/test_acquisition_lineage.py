...
from web_listening.blocks.acquisition_evidence import (
    _portable_source_path,
    persisted_acquisition_evidence,
)
...
def _attempt(**changes) -> AcquisitionAttempt:
    ...
    request = CaptureRequest(
        ...
        metadata={
            ...
            "fallback_position": attempt.position,
            ...
            "entrypoint": None,
            "script_sha256": attempt.script_sha256,
    ...
    result = CaptureResult(
        **lineage,
        state="succeeded" if attempt.accepted else "failed",
        ...
        content=CaptureContent(media_type="text/html", text="content")
        if attempt.accepted
        else None,
        error=None
        if attempt.accepted
        else CaptureError(
            code=attempt.classification,
            message=attempt.reason or attempt.classification,
        ),
        metadata={
            "acquisition_classification": attempt.classification,
...
def test_legacy_database_opens_without_backfill_and_nullable_lineage(tmp_path):
    ...
    assert "attempt_id" in {
        row["name"] for row in storage.conn.execute("PRAGMA table_info(page_snapshots)")
    }
...
def test_attempt_persistence_is_idempotent_and_conflicts_fail_closed(tmp_path):
    ...
    assert [
        item.classification for item in storage.list_acquisition_attempts(1, 1)
    ] == ["accepted"]
    storage.close()


@pytest.mark.parametrize(
    ("classification", "accepted", "reason", "valid"),
    [
        ("accepted", True, "", True),
...
def test_attempt_semantic_acceptance_matrix_rejects_contradictions_before_mutation(
    ...
    assert storage.conn.execute("SELECT COUNT(*) FROM acquisition_attempts").fetchone()[
        0
    ] == int(valid)
    storage.close()
...
def test_rejected_attempt_reason_must_match_classification_before_mutation(tmp_path):
    ...
    with pytest.raises(ValueError, match="classification or reason"):
        storage.add_acquisition_attempt(attempt)
    assert (
        storage.conn.execute("SELECT COUNT(*) FROM acquisition_attempts").fetchone()[0]
        == 0
    )
    storage.close()
...
@pytest.mark.parametrize(
    ("field", "value"), [("position", 99), ("profile_id", "different-profile")]
)
def test_persistence_rejects_relational_authority_that_contradicts_canonical(
    ...
    canonical["request"]["metadata"].update(
        {"profile_id": "profile", "fallback_position": 0}
    )
...
@pytest.mark.parametrize(
    ...
    [
        *(
        ...
        ("result", "acquisition_classification"),
        ("result", "acquisition_validation"),
    ],
)
...
def test_snapshot_requires_accepted_same_run_scope_attempt(tmp_path):
    ...
    scope = storage.add_crawl_scope(
        CrawlScope(site_id=site.id, seed_url=site.url, allowed_origin=site.url)
    )
    run = storage.add_crawl_run(CrawlRun(scope_id=scope.id))
    page = storage.upsert_tracked_page(
        scope_id=scope.id, canonical_url=site.url, depth=0, run_id=run.id
    )
...
def test_page_snapshot_redacts_all_text_surfaces_and_recomputes_hash(tmp_path):
    ...
    assert (
        ...
        == hashlib.sha256(snapshot.fit_markdown.encode()).hexdigest()
    )
    assert (
        snapshot.content_hash
        != hashlib.sha256(surfaces["fit_markdown"].encode()).hexdigest()
    )

    long_text = "A" * 5000
    plain = storage.add_page_snapshot(
        PageSnapshot(
            scope_id=1,
            run_id=1,
            page_id=2,
            ...
            raw_html=long_text,
            cleaned_html=long_text,
            content_text=long_text,
            markdown=long_text,
            fit_markdown=long_text,
            metadata_json={"hash_basis": "fit_markdown"},
        )
    )
    for field in surfaces:
...
def test_governed_failed_attempt_preserves_null_final_url_and_redacts_host_path():
    ...
    result = CaptureResult(
        ...
        site_skill_digest=request.site_skill_digest,
    ...
    attempt = gateway._attempt(
        request,
        {"position": 0, "executor_version": "1.0.0", "script_sha256": None},
        "page",
        "executor_error",
        result,
    )
...
def test_tracked_state_requires_typed_lineage_ownership(tmp_path):
    ...
    document_attempt = _attempt(
        attempt_id="document", request_id="document", content_kind="document"
    )
    ...
    with pytest.raises(ValueError, match="content_kind=page"):
        storage.add_page_snapshot(
            PageSnapshot(
                ...
                run_id=1,
                page_id=1,
                attempt_id=document_attempt.attempt_id,
                content_hash="hash",
            )
        )
    with pytest.raises(ValueError, match="content_kind=document"):
        storage.add_file_observation(
            FileObservation(
                ...
                page_id=1,
                file_id=1,
                attempt_id=page_attempt.attempt_id,
                discovered_url="https://example.com/a.pdf",
...
def test_inline_artifact_admission_verifies_and_publishes_portable_path(tmp_path):
    ...
    artifacts = storage.admit_inline_acquisition_artifacts(
        attempt.attempt_id, [descriptor]
    )
    ...
    assert not list(
        (tmp_path / "acquisition_artifacts" / attempt.attempt_id).glob("*.tmp")
    )
    storage.close()


@pytest.mark.skipif(
    not hasattr(storage_module.os, "O_TMPFILE"),
    reason="requires native unnamed temporary files",
)
@pytest.mark.parametrize("preexisting_directories", [False, True])
def test_nested_inline_artifact_rollback_respects_directory_creation_provenance(
    tmp_path: Path, preexisting_directories: bool
) -> None:
    storage = Storage(tmp_path / "inline-directory-provenance.db")
    storage.add_acquisition_attempt(_attempt())
    attempt_directory = tmp_path / "acquisition_artifacts" / "attempt-1"
    if preexisting_directories:
        attempt_directory.mkdir(parents=True)
    data = b"\x89PNG\r\n\x1a\ninline-directory-provenance"
    descriptor = {
        "kind": "screenshot",
        "mime_type": "image/png",
        "size_bytes": len(data),
    ...
    assert attempt_directory.exists() is preexisting_directories
    assert (tmp_path / "acquisition_artifacts").exists() is preexisting_directories
    storage.close()
...
def test_inline_artifact_keyboard_interrupt_after_named_link_cleans_all_state(
    tmp_path, monkeypatch
):
    storage = Storage(tmp_path / "artifact-interrupt.db")
    storage.add_acquisition_attempt(_attempt())
    baseline_db = storage.db_path.read_bytes()
    data = b"\x89PNG\r\n\x1a\nnative-interrupt-fixture"
    descriptor = {
        "kind": "screenshot",
        "mime_type": "image/png",
        "size_bytes": len(data),
    ...
    failure = KeyboardInterrupt("interrupt after named link")
    storage.begin_execution_transaction()
    monkeypatch.setattr(
        storage,
    ...
    assert (
        storage.conn.execute("SELECT COUNT(*) FROM acquisition_artifacts").fetchone()[0]
        == 0
    )
...
def test_inline_artifact_link_effect_then_interrupt_cleans_pending_owned_inode(
    tmp_path, monkeypatch
):
    storage = Storage(tmp_path / "artifact-link-effect.db")
    storage.add_acquisition_attempt(_attempt())
    baseline_db = storage.db_path.read_bytes()
    data = b"\x89PNG\r\n\x1a\nlink-effect-fixture"
    descriptor = {
    ...
    failure = KeyboardInterrupt("interrupt inside link wrapper")
    real_link = storage._link_unnamed_temporary

    def link_then_interrupt(*args):
        real_link(*args)
        raise failure

    monkeypatch.setattr(storage, "_link_unnamed_temporary", link_then_interrupt)
...
def test_inline_artifact_link_interrupt_preserves_replacement_inode(
    tmp_path, monkeypatch
):
    storage = Storage(tmp_path / "artifact-link-replacement.db")
    storage.add_acquisition_attempt(_attempt())
    data = b"\x89PNG\r\n\x1a\nlink-replacement-fixture"
    descriptor = {
        "kind": "screenshot",
        "mime_type": "image/png",
        "size_bytes": len(data),
    ...
    def link_replace_then_interrupt(source_fd, parent_fd, target):
        ...
        try:
            storage_module.os.write(replacement_fd, replacement)
            info = storage_module.os.fstat(replacement_fd)
            replacement_identity = (info.st_dev, info.st_ino)
        finally:
            storage_module.os.close(replacement_fd)
        raise failure

    monkeypatch.setattr(storage, "_link_unnamed_temporary", link_replace_then_interrupt)
    with pytest.raises(SystemExit) as caught:
        storage.admit_inline_acquisition_artifacts("attempt-1", (descriptor,))

    target = tmp_path / "acquisition_artifacts" / "attempt-1" / "00-screenshot.png"
...
def test_inline_artifact_replacement_before_real_journal_handoff_is_never_claimed(
    tmp_path, monkeypatch, replacement_kind
):
    storage = Storage(tmp_path / f"artifact-handoff-{replacement_kind}.db")
    storage.add_acquisition_attempt(_attempt())
    baseline_db = storage.db_path.read_bytes()
    storage.begin_execution_transaction()
    data = b"\x89PNG\r\n\x1a\ninline-handoff-fixture"
    descriptor = {
        "kind": "screenshot",
        "mime_type": "image/png",
        "size_bytes": len(data),
...
def test_inline_artifact_commit_base_exception_reconciles_durable_ownership(
    tmp_path, commit_effected
):
    storage = Storage(tmp_path / f"artifact-commit-{commit_effected}.db")
    storage.add_acquisition_attempt(_attempt())
    real_connection = storage.conn
    data = b"\x89PNG\r\n\x1a\ncommit-effect-fixture"
    descriptor = {
        "kind": "screenshot",
        "mime_type": "image/png",
        "size_bytes": len(data),
    ...
    failure = SystemExit(31 if commit_effected else 32)
    ...
    ).fetchone()[0] == int(commit_effected)
...
def test_inline_artifact_system_exit_before_staging_closes_and_cleans(
    tmp_path, monkeypatch
):
    storage = Storage(tmp_path / "artifact-system-exit.db")
    storage.add_acquisition_attempt(_attempt())
    data = b'{"event":"visible"}'
    descriptor = {
        "kind": "trace",
...
def test_inline_artifact_interrupt_preserves_preexisting_exact_inode(
    tmp_path, monkeypatch
):
    storage = Storage(tmp_path / "artifact-preexisting-interrupt.db")
    storage.add_acquisition_attempt(_attempt())
    data = b"\x89PNG\r\n\x1a\npreexisting-fixture"
    descriptor = {
        "kind": "screenshot",
    ...
    target = tmp_path / artifact.portable_path
    before = target.stat(follow_symlinks=False)
    before_rows = storage.conn.execute(
        "SELECT * FROM acquisition_artifacts ORDER BY portable_path"
    ).fetchall()
    failure = KeyboardInterrupt("preexisting target")
    monkeypatch.setattr(
        storage,
        "_verify_existing_artifact",
        lambda *args: (_ for _ in ()).throw(failure),
...
def test_textual_artifact_is_structurally_redacted_before_publication_and_row_commit(
    tmp_path,
):
    storage = Storage(tmp_path / "artifact-redaction.db")
    storage.add_acquisition_attempt(_attempt())
    original = (
        b'{"headers":{"Authorization":"Bearer ARTIFACT-SECRET-CANARY"},"ok":"visible"}'
    )
    descriptor = {
        "kind": "trace",
        "mime_type": "application/json",
    ...
    assert (
        row["redaction_status"] == artifact.redaction_status == "structurally_redacted"
    )
...
def test_raw_capture_html_embedded_fragment_credentials_are_redacted_with_truthful_metadata(
    tmp_path,
):
...
def test_unknown_attempt_artifact_rejected_before_filesystem_or_database_mutation(
    tmp_path,
):
...
def test_artifact_creation_faults_leave_no_temp_or_fd_leak(
...
def test_inline_artifact_replay_is_conflict_safe_and_accepts_frozen_capture_sequence(
    tmp_path,
):
    storage = Storage(tmp_path / "artifact-replay.db")
    first = _attempt(
        attempt_id="failed-first",
        request_id="failed-first",
        accepted=False,
        classification="blocked",
    )
    second = _attempt(
        attempt_id="accepted-second", request_id="accepted-second", position=1
    )
    ...
    outcome = AcquisitionOutcome(
        ...
        True,
        (first, second),
        ((descriptor(data1),), tuple(result.metadata["inline_artifacts"])),
    )
    TreeCrawler(
        storage=storage, acquisition_gateway=object()
    )._persist_acquisition_outcome(
        outcome,
        requested_url="https://example.com/a",
        run_id=1,
        scope_id=1,
...
def test_inline_artifact_attempt_id_rejects_nonportable_components(
    tmp_path, attempt_id
):
...
def test_persistence_recomputes_redaction_for_relational_and_canonical_fields(tmp_path):
    ...
    attempt = _attempt(
        ...
        accepted=False,
        reason="blocked",
        validation={"authorization": "Bearer abc", "diagnostic": "password=hunter2"},
    ...
    assert (
        "top-secret" not in serialized
        and "hunter2" not in serialized
        and "Bearer abc" not in serialized
    )
    assert "caller-lied" not in serialized and "%5BREDACTED%5D" in stored.requested_url
    with pytest.raises(ValueError, match="non-null"):
        storage.add_page_snapshot(
            PageSnapshot(scope_id=1, run_id=1, page_id=1, content_hash="x")
        )
    storage.close()
...
def test_structural_redaction_handles_tuples_compound_keys_and_url_userinfo(tmp_path):
    ...
    attempt = _attempt(
        ...
        validation={
            "nested": ({"serviceCredentialValue": "token-canary"},),
            "ordinary": ("visible",),
        },
        authority_mode="legacy_compatibility",
    )
    storage.add_acquisition_attempt(attempt)
    relational = json.dumps(
        dict(storage.conn.execute("SELECT * FROM acquisition_attempts").fetchone())
    )
...
def test_url_fragment_credentials_are_redacted_on_relational_canonical_and_export_surfaces(
    tmp_path,
):
    storage = Storage(tmp_path / "fragment-redaction.db")
    secret_url = (
        "https://example.com/callback#access_token=fragment-canary&state=visible"
    )
    stored = storage.add_acquisition_attempt(
        _attempt(requested_url=secret_url, final_url=secret_url)
    )
    relational = json.dumps(
        dict(storage.conn.execute("SELECT * FROM acquisition_attempts").fetchone())
    )
...
def test_fragment_credential_carrier_key_is_fully_redacted_without_losing_ordinary_fragment(
    tmp_path,
):
    storage = Storage(tmp_path / "fragment-carrier-redaction.db")
    secret_url = (
        "https://example.com/callback#access_token/FRAGMENT-SECRET-CANARY&state=visible"
    )
    stored = storage.add_acquisition_attempt(
        _attempt(requested_url=secret_url, final_url=secret_url)
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("application/zip", b"not zip"),
    ],
)
def test_inline_artifact_rejects_bytes_that_do_not_match_declared_mime(
    tmp_path, mime, data
):
    storage = Storage(tmp_path / "mime.db")
    storage.add_acquisition_attempt(_attempt())
    descriptor = {
        "kind": "screenshot" if mime.startswith("image/") else "trace",
        "mime_type": mime,
        "size_bytes": len(data),
        "sha256": hashlib.sha256(data).hexdigest(),
        "data_base64": base64.b64encode(data).decode(),
...
def test_persisted_canonical_attempt_validates_against_frozen_contract(tmp_path):
    ...
    ContractAcquisitionAttempt.model_validate_json(
        json.dumps(evidence["attempts"][0]["canonical_attempt"])
    )
    storage.close()


def test_artifact_parent_replacement_cannot_escape_pinned_tree_or_commit_row(
    tmp_path, monkeypatch
):
...
def test_pinned_directories_are_reverified_after_publication_before_commit(
    tmp_path, monkeypatch
):
    ...
    monkeypatch.setattr(
        storage, "_verify_pinned_artifact_directories", replacing_verify
    )
    with pytest.raises(ValueError, match="changed during publication"):
        storage.admit_inline_acquisition_artifacts("attempt-1", (descriptor,))
    assert not (tmp_path / "detached-attempt" / "00-trace.json").exists()
    assert not (
        tmp_path / "acquisition_artifacts" / "attempt-1" / "00-trace.json"
    ).exists()
    assert (
        storage.conn.execute("SELECT COUNT(*) FROM acquisition_artifacts").fetchone()[0]
        == 0
...
def test_artifact_model_fault_after_temp_open_closes_fd_and_removes_exact_temp(
    tmp_path, monkeypatch
):
...
def test_first_temp_fstat_failure_closes_fd_and_preserves_named_replacement(
    tmp_path, monkeypatch
):
    ...
    assert not list(tmp_path.rglob("*.tmp"))
    assert (
        tmp_path / "acquisition_artifacts" / "attempt-1" / "replacement.bin"
...
def test_named_target_replacement_during_verification_fails_without_false_digest(
    tmp_path, monkeypatch
):
...
def test_parent_replacement_after_target_verification_fails_before_commit(
    tmp_path, monkeypatch
):
...
def test_attempt_directory_replacement_after_final_target_verifier_rolls_back_rows_and_owned_bytes(
    ...
    monkeypatch.setattr(
        storage, "_verify_existing_artifact", replacing_after_final_verify
    )
    with pytest.raises(ValueError, match="changed during publication"):
        storage.admit_inline_acquisition_artifacts("attempt-1", (descriptor,))
    assert calls == 2
...
def test_target_replacement_after_initial_verification_fails_before_commit(
    tmp_path, monkeypatch
):
...
def test_compatibility_source_paths_replace_nonportable_basenames_deterministically(
    basename,
):
...
def test_exact_run_chronology_drives_latest_evidence_not_accepted_attempt():
    ...
    timeout = _attempt(
        ...
        request_id="chain-b",
        position=0,
        requested_at=base.replace(second=1),
        started_at=base.replace(second=1),
        finished_at=base.replace(second=1),
        accepted=False,
        classification="timeout",
    )
    for attempt in (timeout, accepted, blocked):
        storage.add_acquisition_attempt(attempt)
    assert [item.attempt_id for item in storage.list_acquisition_attempts(1, 1)] == [
        "blocked",
        "accepted",
        "timeout",
    ]
...
def test_legacy_compatibility_replay_ignores_volatile_timestamps_but_conflicts_semantically(
    tmp_path,
):
    ...
    storage.conn.execute(
        "UPDATE acquisition_attempts SET content_kind='document' WHERE attempt_id=?",
        (first.attempt_id,),
    )
    storage.conn.commit()
...
def test_legacy_exception_attempt_is_redacted_and_labeled():
    ...
    outcome = LegacyCrawlerGateway(
        Crawler(), fetch_mode="http", fetch_config_json={}
...
def test_legacy_runtime_requires_complete_canonical_authority_before_mutation(
    tmp_path, field
):
    class Crawler:
        def fetch_page(self, *args, **kwargs):
    ...
    attempt = (
        LegacyCrawlerGateway(Crawler(), fetch_mode="http", fetch_config_json={})
        .acquire("https://example.com/", run_id="1", scope_id="2")
        .attempt_records[0]
    )
    canonical = json.loads(attempt.canonical_json)
