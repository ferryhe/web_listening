...
def test_execution_plan_preview_plural_api_uses_stable_compatibility_envelope(
...
""",
    ...
    assert (
        client.post(
            "/api/v1/acquisition/execution-plan/preview",
            json={"scope_path": "scope.yaml"},
        ).status_code
        == 404
    )


def test_execution_plan_preview_pins_validated_scope_before_parsing(
    tmp_path: Path, monkeypatch
):
    monkeypatch.setattr(routes.settings, "data_dir", str(tmp_path))
    scope = tmp_path / "scope.yaml"
...
def test_execution_plan_preview_rejects_symlinked_input_components(
    ...
    assert response.json() == {
        ...
        "error": {
            "code": "input.invalid",
            "field": ".",
            "message": "preview input is invalid",
        },
    }
...
def test_execution_plan_preview_rejects_fifo_input_promptly(
    tmp_path: Path, monkeypatch
):
    monkeypatch.setattr(routes.settings, "data_dir", str(tmp_path))
...
def test_execution_plan_preview_api_structured_redacted_failure(
    tmp_path: Path, monkeypatch
):
    monkeypatch.setattr(routes.settings, "data_dir", str(tmp_path))
    scope = tmp_path / "scope.yaml"
...
def test_execution_plan_preview_api_malformed_scope_yaml_is_redacted_422(
    tmp_path: Path, monkeypatch
):
    monkeypatch.setattr(routes.settings, "data_dir", str(tmp_path))
...
def test_execution_plan_preview_api_sequence_scope_root_is_redacted_422(
    tmp_path: Path, monkeypatch
):
    monkeypatch.setattr(routes.settings, "data_dir", str(tmp_path))
    scope = tmp_path / "SECRET-PATH-CANARY-scope.yaml"
...
def test_execution_plan_preview_api_missing_path_is_redacted_envelope(
    tmp_path: Path, monkeypatch
):
    monkeypatch.setattr(routes.settings, "data_dir", str(tmp_path))
    response = TestClient(create_app()).post(
...
def test_execution_plan_preview_api_rejects_coerced_scope_limits(
    tmp_path: Path, monkeypatch, limit_yaml: str
):
    monkeypatch.setattr(routes.settings, "data_dir", str(tmp_path))
...
def test_execution_plan_preview_api_rejects_coerced_profile_authority(
    tmp_path: Path, monkeypatch
):
    monkeypatch.setattr(routes.settings, "data_dir", str(tmp_path))
    scope = tmp_path / "scope.yaml"
...
def test_execution_plan_preview_route_owns_all_request_validation(
    tmp_path: Path, monkeypatch
):
    monkeypatch.setattr(routes.settings, "data_dir", str(tmp_path))
    client = TestClient(create_app())
...
def test_execution_plan_preview_owns_malformed_json_and_openapi_schema():
    ...
    assert operation["responses"]["200"]["content"]["application/json"]["schema"][
        "$ref"
    ].endswith("/AcquisitionExecutionPreviewResponse")
...
def test_acquisition_probe_endpoint_loads_profile_path_without_site_key(
...
def test_acquisition_probe_endpoint_rejects_profile_path_with_inline_overrides(
    tmp_path, monkeypatch
):
    monkeypatch.setattr(routes.settings, "data_dir", tmp_path)
    profile_path = tmp_path / "profiles" / "profile.yaml"
    ...
    assert response.status_code == 422
    assert (
        "inline override fields are not allowed with profile_path"
...
def test_acquisition_probe_endpoint_rejects_cloakbrowser_without_authorization(
    monkeypatch,
):
    monkeypatch.setattr(
        "web_listening.blocks.acquisition_tools.build_builtin_adapters",
...
def test_acquisition_probe_endpoint_rejects_profile_path_outside_data_dir(
    tmp_path, monkeypatch
):
    monkeypatch.setattr(routes.settings, "data_dir", tmp_path)
...
def test_acquisition_probe_endpoint_requires_site_key_without_profile_path():
    ...
    assert response.status_code == 422
    assert (
        "site_key is required when profile_path is not provided"
        in response.json()["detail"]
    )
...
def test_get_latest_snapshot_endpoint_returns_404_without_snapshot(
    tmp_path, monkeypatch
):
    db_path = tmp_path / "api.db"
    monkeypatch.setattr(routes.settings, "db_path", db_path)
...
def test_rescue_check_endpoint_is_disabled_before_storage(monkeypatch):
    def forbidden(*args, **kwargs):
        raise AssertionError("disabled legacy rescue reached target-read state")

    monkeypatch.setattr(routes, "get_storage", forbidden)
    monkeypatch.setattr(routes, "run_site_rescue", forbidden)

    response = TestClient(create_app()).post(
        "/api/v1/sites/1/rescue-check",
        json={"allow_browser": True, "allow_official_feeds": True},
    )

    assert response.status_code == 409
    assert "governed scoped execution" in response.json()["detail"]


def test_disabled_rescue_check_does_not_validate_candidate_urls(monkeypatch):
    monkeypatch.setattr(
        routes,
        "_validate_url",
        lambda *args, **kwargs: (_ for _ in ()).throw(
            AssertionError("disabled rescue inspected a candidate URL")
        ),
    )
...
def test_create_monitor_task_endpoint_rejects_output_path_outside_data_dir(
    tmp_path, monkeypatch
):
    db_path = tmp_path / "api.db"
    monkeypatch.setattr(routes.settings, "db_path", db_path)
...
def test_scope_bootstrap_job_endpoint_persists_completed_job(tmp_path, monkeypatch):
    ...
    def fake_bootstrap_scope(**kwargs):
        ...
        return SimpleNamespace(
            results=[SimpleNamespace(scope_id=scope.id, run_id=run.id)],
    ...
    monkeypatch.setattr(
        routes,
        "_prepare_scope_execution_authority",
        lambda scope_id, **kwargs: SimpleNamespace(
            scope_path=Path(kwargs["scope_path"]),
            acquisition_profile_path=Path(kwargs["acquisition_profile_path"]),
            close=lambda: None,
        ),
    )
    ...
    client = TestClient(create_app())
    response = client.post(
        f"/api/v1/monitor-scopes/{scope.id}/bootstrap",
        json={
            "scope_path": str(scope_path),
            "acquisition_profile_path": str(tmp_path / "profile.yaml"),
        },
    )
...
def test_scope_bootstrap_endpoint_uses_explicit_authorized_scope_path(
    tmp_path, monkeypatch
):
    ...
    def fake_bootstrap_scope(**kwargs):
        seen["scope_path"] = str(kwargs["prepared"].scope_path)
        report_path = tmp_path / "reports" / "bootstrap-fingerprint.md"
        report_path.parent.mkdir(parents=True, exist_ok=True)
        report_path.write_text("# Bootstrap\n", encoding="utf-8")
        return SimpleNamespace(
            results=[SimpleNamespace(scope_id=scope_two.id, run_id=123)],
            report_path=report_path,
            summary_path=None,
        )
    ...
    client = TestClient(create_app())
    response = client.post(
        f"/api/v1/monitor-scopes/{scope_two.id}/bootstrap",
        json={
            "scope_path": str(plans_dir / "monitor_scope_two.yaml"),
            "acquisition_profile_path": str(tmp_path / "profile.yaml"),
        },
    )
...
def test_scope_run_job_endpoint_persists_completed_job(tmp_path, monkeypatch):
    ...
    run = storage.add_crawl_run(
        CrawlRun(scope_id=scope.id, run_type="incremental", status="completed")
    )
    storage.close()
    ...
    def fake_run_scope(**kwargs):
        ...
        return SimpleNamespace(
            result=SimpleNamespace(scope_id=scope.id, run_id=run.id),
            report_path=report_path,
        )
    ...
    client = TestClient(create_app())
    response = client.post(
        f"/api/v1/monitor-scopes/{scope.id}/run",
        json={
            "scope_path": str(scope_path),
            "acquisition_profile_path": str(tmp_path / "profile.yaml"),
        },
...
def test_scope_report_job_and_latest_report_endpoint(tmp_path, monkeypatch):
    ...
    run = storage.add_crawl_run(
        CrawlRun(
            scope_id=scope.id, run_type="incremental", status="completed", pages_seen=2
        )
    )
    storage.update_crawl_scope(
        CrawlScope(
            **{**scope.model_dump(), "baseline_run_id": run.id, "is_initialized": True}
        )
    )
    storage.close()
...
""".strip()
    ...
    def fake_report_scope(**kwargs):
        ...
        return SimpleNamespace(
            report=report_payload, output_path=report_path, output_format="md"
        )
    ...
    assert (
        delivery_payload["artifact_contract"]["contract_version"]
    ...
    assert delivery_payload["artifact_contract"]["path_map"]["output_path"] == str(
        report_path
    )
    ...
    assert (
        latest_payload["report_payload"]["next_action"] == "review_low_priority_changes"
    )
    report_artifact = next(
        item
        for item in latest_payload["report_payload"]["artifact_index"]
        if item["kind"] == "tracking_report"
    )
    assert report_artifact["path"] == str(report_path)
    assert report_artifact["recommended_reader"] == "markdown"


def test_scope_report_endpoint_guards_and_passes_acquisition_paths(
    tmp_path, monkeypatch
):
    db_path = tmp_path / "api.db"
    monkeypatch.setattr(routes.settings, "db_path", db_path)
    ...
    run = storage.add_crawl_run(
        CrawlRun(
            scope_id=scope.id, run_type="incremental", status="completed", pages_seen=1
        )
    )
    storage.update_crawl_scope(
        CrawlScope(
...
""".strip()
    ...
    def fake_report_scope(**kwargs):
        ...
        report_path.write_text("# Demo report\n", encoding="utf-8")
        return SimpleNamespace(
            report=SimpleNamespace(run_id=run.id),
    ...
    assert response.status_code == 201
    payload = response.json()
    assert payload["produced_artifacts"]["acquisition_profile_path"] == str(
        profile_path.resolve()
    )
    assert payload["produced_artifacts"]["capture_attempt_path"] == str(
        capture_attempt_path.resolve()
    )


def test_scope_report_endpoint_rejects_acquisition_path_outside_data_dir(
    tmp_path, monkeypatch
):
    db_path = tmp_path / "api.db"
    monkeypatch.setattr(routes.settings, "db_path", db_path)
...
def test_scope_report_endpoint_rejects_acquisition_directory_path(
    tmp_path, monkeypatch
):
    db_path = tmp_path / "api.db"
    monkeypatch.setattr(routes.settings, "db_path", db_path)
...
""".strip()
    ...
    client = TestClient(create_app())
    response = client.post(
        f"/api/v1/monitor-scopes/{scope.id}/report", json={"output_format": "json"}
    )
...
def test_scope_report_latest_endpoint_rejects_artifact_outside_data_dir(
    tmp_path, monkeypatch
):
    db_path = tmp_path / "api.db"
    monkeypatch.setattr(routes.settings, "db_path", db_path)
...
""".strip()
    ...
    def fake_report_scope(**kwargs):
        ...
        return SimpleNamespace(
            report=report_payload, output_path=report_path, output_format="yaml"
        )
    ...
    client = TestClient(create_app())
    response = client.post(
        f"/api/v1/monitor-scopes/{scope.id}/report", json={"output_format": "yaml"}
    )
    ...
    assert (
        latest_payload["report_payload"]["artifact_index"][0]["recommended_reader"]
        == "yaml"
    )


def test_scope_report_endpoint_rejects_task_path_outside_data_dir(
    tmp_path, monkeypatch
):
    db_path = tmp_path / "api.db"
    monkeypatch.setattr(routes.settings, "db_path", db_path)
...
def test_scope_manifest_latest_endpoint_generates_and_persists_job(
    tmp_path, monkeypatch
):
    db_path = tmp_path / "api.db"
    monkeypatch.setattr(routes.settings, "db_path", db_path)
