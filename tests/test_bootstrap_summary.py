...
""".strip()
    ...
    try:
        ...
        page_b = storage.upsert_tracked_page(
            scope_id=scope.id,
            canonical_url="https://example.com/publications/books",
            depth=1,
            run_id=run.id,
        )
        snap_b = storage.add_page_snapshot(
            PageSnapshot(
                ...
                run_id=run.id,
                content_hash="hash-b",
                final_url="https://example.com/publications/books",
            )
        )
        storage.upsert_tracked_page(
            scope_id=scope.id,
    ...
    assert level1["/research"] == (1, 2)
    assert level1["/publications"] == (1, 1)
    assert level2["/research/topics"] == (1, 2)
    assert level2["/publications/books"] == (1, 1)
