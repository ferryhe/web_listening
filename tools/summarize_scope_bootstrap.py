...
def build_default_report_path(site_key: str, now: datetime | None = None) -> Path:
    ...
    return settings.data_dir / "reports" / f"bootstrap_scope_summary_{site_key}_{report_date}.md"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarize a completed scope bootstrap by level-1 and level-2 directories.")
...
def main() -> None:
    ...
    try:
        summary = summarize_monitor_scope_bootstrap(args.scope_path, storage=storage, run_id=args.run_id)
    finally:
        storage.close()
