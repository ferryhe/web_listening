...
class HttpAcquisitionAdapter:
    ...
    def capture(self, url: str, *, config: dict | None = None) -> FetchResult:
        try:
            ...
            page = normalize_html(response.text, base_url=str(response.url))
            metadata = dict(page.metadata)
            metadata["driver"] = "http"
            metadata["request_user_agent"] = response.request.headers.get(
                "User-Agent", resolve_request_headers(config).get("User-Agent", settings.user_agent)
            )
            metadata["http_status_error"] = True
            return FetchResult(page.raw_html, page.cleaned_html, page.content_text, page.markdown, page.fit_markdown, metadata, str(response.url), response.status_code)
